...
Build a function called keyMatcher() which, when passed two objects and a key, will use the string to look up the key-value pair in each object and compare the values. If the two values are explicitly equal to each other, return true, otherwise return false if either the values or not the same, or both objects do not have that key.
...
const keyMatcher = function(firstObj, secondObj, key) {
  let obj1Keys = Object.keys(firstObj);
  let obj2Keys = Object.keys(secondObj);
  let match = '';
  ...
  if (firstObj[match] === secondObj[match]) {
    return true;
  } else {
    return false;
  }
